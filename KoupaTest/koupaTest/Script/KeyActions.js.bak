...
function ClickCell(ObjGrid, RowIndex, ColName) {
    ...
    //ColumnIndex = ObjGrid.GetColumnIndex(Index);
    ColumnIndex = FindColumnIndex(ObjGrid, ColName)

    if (ColumnIndex == -1) {
      
        Log.Error("ClickCell: Column '" + ColName + "' not found " + " in ObjGrid : " + ObjGrid.Name);
...
function FindRowByCellValue(ObjGrid, ColName, Value) {
    ...
    //let ColumnIndex = ObjGrid.GetColumnIndex(ColumnName);
    let  ColumnIndex = FindColumnIndex(ObjGrid, ColName)
    
    if (ColumnIndex == -1) {
      
        Log.Error("FindRowByCellValue: Column '" + ColName + "' not found " + " in Grid : " + ObjGrid.Name);
    } else {
...
function CheckpointGridValue(ObjGrid, RowIndex, ColName, Value) {
    ...
    if (columnIndex == -1) {
        ...
        //aqObject.CheckProperty(ObjGridItem.FormattedText, "OleValue", cmpEqual, value);
