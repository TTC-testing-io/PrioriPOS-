...
function GetElement(ObjectRepo, ApplicationName, FormName, ObjectType, ObjectName) {
    ...
    ObjForm = WaitForFormReady(ApplicationName, OrFormName) 

    ObjElement = ObjForm.FindChild("WinFormsControlName", ORObjectName, 15, true);
...
function WaitForFormReady(ApplicationName, OrFormName) {

    let TimeOut = 90000;
    let Interval = 250;
    let Elapsed = 0;

    let ObjApplication = Sys.Process(ApplicationName);

    while (Elapsed < TimeOut) {

        let ObjForm = ObjApplication.WinFormsObject(OrFormName)
        
        if (ObjForm != null &&  ObjForm != undefined) {
          
            if (ObjForm.Visible === true && ObjForm.Enabled === true) {
...
function WaitForElementReady(ObjElement, ElementIsEnabled) {
    ...
    while (Elapsed < TimeOut) {
        ...
        if (ObjElement != null && ObjElement.Exists) {
            ...
            if (ElementIsEnabled === true) {
                ...
                if (ObjElement.Visible === true && ObjElement.Enabled === true) {
    ...
    return null; // ✅ retourne null si l’élément n’est jamais prêt
}
